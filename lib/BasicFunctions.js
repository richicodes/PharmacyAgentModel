...
// Question to Richi for the picker/dispenser functions
// is it possible to use the .push method as before to update
// the relevant variables?


//update picker function
function updatePicker (prescriptionID){
    prescriptionID = Number(prescriptionID); 
    var prescription = prescription[prescriptionID];
    var state = prescription.state;

    if (state == PRESCRIPTION_DELIVERYQUEUE){
        // insert removal action
    }
    else {
        switch(state){
...
/* Just parking this here temporarily - using this as reference to 
   code the functions above and below
...
// updateDispenser function
// note: dispenser
function updateDispenser (state, whoseID){ //whoseID here is meant to hold the identifier for which prescription number we're dispensing -> Richi, is this correct?
    //insert code to grab dispenser state here
    // probably use filter.d outside of this function
    //  again to grab the correct state? need to check with richi
    switch(state){
    case DISPENSER_AVAILABLE:
        //call for next prescription
        var dispensernow = {"case": whoseID, "state": DISPENSER_WAITING};
        dispenser.push(dispensernow);
        break;

    case DISPENSER_WAITING:
        //awaiting next prescription
        // conditional check here: sees if patient misses their turn
        if(Math.random()<ProbReact){ 
            var dispensernow = {"case": whoseID, "state": DISPENSER_BUSY};}
        else 
            var dispensernow = {"case": whoseID, "state": DISPENSER_AVAILABLE};
        dispenser.push(dispensernow);
        break;

    case DISPENSER_BUSY:
        //currently in the process of dispensing medicine
        // question: would it be good to insert a time delay(normally distributed) here?
        var dispensernow = {"case": whoseID, "state": DISPENSER_AVAILABLE};
        dispenser.push(dispensernow);
        break;
    }
...
// update mdispenser function
function updateMDispenser (state, whoseID){
    switch(state){
    case MDISPENSER_AVAILABLE:
        var mdispensernow = {"case": whoseID, "state": MDISPENSER_BUSY};
        mdispenser.push(mdispensernow); // update the mdispenser variable with the ID of prescription, and state of dispenser
        break;

    case MDISPENSER_BUSY:
        //busy generating prescription
        // question: would it be good to insert a time delay(normally distributed) here? -> same as above
        var mdispensernow = {"case": whoseID, "state": MDISPENSER_AVAILABLE};
        mdispenser.push(mdispensernow);
        break;    
    }
}
...
function updatePrescription(prescriptionID){
    ...
    switch(state){
        ...
        case PRESCRIPTION_REGISTERED:
            // William: quick question here: do we need to push this to the prescriptions variable?
