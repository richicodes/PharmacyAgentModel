...
var areas =[
//{"label":"Waiting Area","startRow":3,"numRows":4,"startCol":15,"numCols":12,"color":"violet"}, // bigger version for fun
//{"label":"Waiting Area","startRow":4,"numRows":1,"startCol":20,"numCols":1,"color":"cyan"}, // tiny version to see rejections
{"label":"Waiting Area","startRow":4,"numRows":3,"startCol":19,"numCols":3,"color":"pink"}, // as given in HW2
...
/*
Part 2 To-Do:
1. Create array to hold coordinates with seats - OK
2. Define waiting area - OK
3. Modify target location (in waiting area) to ensure that only 1 person sits down per seat  - OK, done in WAITING case
4. VERY IMPORTANT QUIRK: if the waiting room is full, we just outright discharge the patient  - OK, implemented with if-else
*/



// create 2D array which holds the coordinates of the seats, as well as whether they are occupied:
var waitingarea = [];
var loop_col_start = areas[0].startCol; //grab the various parameters as outlined in HW2
var loop_row_start = areas[0].startRow;
var loop_col_max = loop_col_start + areas[0].numCols;
var loop_row_max = loop_row_start + areas[0].numRows;

const UNOCCUPIED = 0, OCCUPIED = 1; // create indicators for the seat

// for loop to fill up the seat array
for (seatcol = loop_col_start; seatcol < loop_col_max; seatcol++)
{
	for (seatrow = loop_row_start; seatrow < loop_row_max; seatrow++)
    {
		var seatstate={"row": seatrow, "col": seatcol, "state": UNOCCUPIED}
        waitingarea.push(seatstate); // append the coordinates of the seat along with its current state to the seat list
    }
}

var currentTime = 0;
var statistics = [
{"name":"Average time in clinic, Type A: ","location":{"row":doctorRow+5,"col":doctorCol-4},"cumulativeValue":0,"count":0},
{"name":"Average time in clinic, Type B: ","location":{"row":doctorRow+6,"col":doctorCol-4},"cumulativeValue":0,"count":0},
{"name":"Average time between rejected patients: ", "location":{"row":doctorRow+7,"col":doctorCol-5},"cumulativeValue":0,"count":0}
];

var rejectedpatients = [
{"name":"Total Number of Rejected Patients: ", "location":{"row":doctorRow+3,"col":doctorCol-4},"cumulativeValue":0,"count":0}
];
...
function updateSurface(){
	...
	//copy-pasted and modified from original for the rejected patients statistics
	// Here we will create a group for each element of the statistics array (two elements)
	var allrejected = surface.selectAll(".rejectedpatients").data(rejectedpatients);
	var newrejected = allrejected.enter().append("g").attr("class","rejectedpatients");
	// For each new statistic group created we append a text label
	newrejected.append("text")
	.attr("x", function(d) { var cell= getLocationCell(d.location); return (cell.x+cellWidth)+"px"; })
    .attr("y", function(d) { var cell= getLocationCell(d.location); return (cell.y+cellHeight/2)+"px"; })
    .attr("dy", ".35em")
    .text(""); 	
...
function addDynamicAgents(){
	...
	if (Math.random()< probArrival){
		...
		"target":{"row":receptionistRow,"col":receptionistCol},"state":UNTREATED,"timeAdmitted":0, "seat":{"row":0,"col":0}};
		if (Math.random()<probTypeA) newpatient.type = "A";
		else newpatient.type = "B";			
		patients.push(newpatient);
	}
	// seat information added to the patient's data
}
...
function updatePatient(patientIndex){
	...
	var rejectedcount=0; // keep track of rejected patient total
	...
	switch(state){
		...
		case UNTREATED:
			if (hasArrived){
				// find out how many empty seats are available
                var emptyseats = waitingarea.filter(function(d){return d.state == UNOCCUPIED});
				//console.log(emptyseats)
                if (emptyseats.length > 0){
					patient.timeAdmitted = currentTime;
					patient.state = WAITING;

                    // pick a random empty seat in the waiting area to queue
                    var emptyseatsindex = Math.floor(Math.random()*emptyseats.length)
                    var patient_seat = emptyseats[emptyseatsindex];
                    
                    // update state of waitingarea
                    var occupied_index = waitingarea.findIndex(function(d){return d.row==patient_seat.row && d.col==patient_seat.col})
                    waitingarea[occupied_index].state = OCCUPIED
					
					// direct patients to move to a given seat coordinate
                    patient.target.row = patient_seat.row;
					patient.target.col = patient_seat.col;

					// update their new seat position to their assigned seat location taken from the empty seat index
					patient.seat.row = patient_seat.row;
					patient.seat.col = patient_seat.col;
					...
					statistics[2].count++;	
					statistics[2].cumulativeValue = 0; //statistics 2 is used to hold time elapsed since last rejection
					//console.log(rejectedpatients[0].count) // keep track of how many patients are rejected
					rejectedpatients[0].count++;
					var timeInClinic = currentTime - patient.timeAdmitted;
					statistics[2].cumulativeValue += timeInClinic;
					// statistics[2].cumulativeValue++;
                }
			}
		break;

/* For the purpose of Part (B), I feel that it's quite pointless to differentiate between 
type A and B patients, but it's not very time-efficient to modify the entire program to simplify the logic.
Hence, a compromise is reached - we piggyback off the original structure by adding 1 simple conditional check:
- If (the patient's current target is at their seat), we change the seat queueing index, and get them to vacate the seat.
*/
			...
			switch (type){
				case "A":
					if (patient.id == nextTreatedPatientID_A){ 
						if (patient.target.col == patient.seat.col && patient.target.row == patient.seat.row){ // check if a patient is on their seat
							var prev_occupied_index = waitingarea.findIndex(function(d){return d.row==patient.seat.row && d.col==patient.seat.col})
							waitingarea[prev_occupied_index].state = UNOCCUPIED // recognise that the patient has left their seat, and update the seat status accordingly
						} 
					...
					if (patient.id == nextTreatedPatientID_A+1){ // check for the next patient in the queue
						if (patient.target.col == patient.seat.col && patient.target.row == patient.seat.row){
							var prev_occupied_index = waitingarea.findIndex(function(d){return d.row==patient.seat.row && d.col==patient.seat.col})
							waitingarea[prev_occupied_index].state = UNOCCUPIED
						}
						patient.target.row = doctorRow-1; //get the patient to stand in the queue, away from the current patient next in line
